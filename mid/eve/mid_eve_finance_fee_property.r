...
data_check <- property_fix %>% 
  distinct(project_name , pk_house , house_code , pk_chargebills , wy_cycle) %>% 
  left_join(accrued) %>% 
  left_join(real) %>% 
  left_join(adjust) %>% 
  left_join(match)
...
print(now())
data_check_year <- data_check %>% 
  mutate(period = case_when(wy_cycle == 0 ~ quarter(cost_month_start) ,
                            wy_cycle == 1 & month(cost_month_start) < 7 ~ as.integer(1) ,
                            wy_cycle == 1 & month(cost_month_start) >= 7 ~ as.integer(2)) ,
         year = year(cost_month_start)) %>% 
  group_by(project_name , pk_house , house_code , wy_cycle , year , period) %>% 
...
data_check_year2 <- data_check_year %>% 
  group_by(project_name , wy_cycle , year , period) %>% 
  summarise(house_cnt = n_distinct(house_code) ,
            done_cnt = n_distinct(house_code[owe <= 0]) , 
            accrued = sum(accrued) ,
            real = sum(real) ,
            adjust = sum(adjust) ,
            match = sum(match) , 
...
cs <- write.xlsx(data_check_year2 , glue('..\\data\\mid\\eve\\数据核对总.xlsx'))  

data_check_year3 <- data_check %>% 
  mutate(year = year(cost_month_start)) %>% 
  group_by(project_name , pk_house , house_code , wy_cycle , year) %>% 
  summarise(accrued = sum(accrued_amount) ,
            real = sum(real_amount) ,
            adjust = sum(adjust_amount) ,
            match = sum(match_amount)) %>% 
  mutate(owe = round(accrued - real - adjust - match , 2)) %>% 
  ungroup() %>% 
  group_by(project_name , wy_cycle , year) %>% 
  summarise(house_cnt = n_distinct(house_code) ,
            done_cnt = n_distinct(house_code[owe <= 0]) , 
            accrued = sum(accrued) ,
            real = sum(real) ,
...
cs <- write.xlsx(data_check_year3 , glue('..\\data\\mid\\eve\\数据核对总2.xlsx'))  

cs <- property_fix %>% 
  filter(pk_project == '0033EDE2FC00F77B77E1')
cs <- write.xlsx(cs , glue('..\\data\\mid\\eve\\数据核对.xlsx'))  
