...
if(nrow(need_split) > 0) {
  ...
  for (i in 1:300){#nrow(need_split)) {
    ...
    # 拆解月份的先写入excel核对
    # 测试了300行数据无问题
    # cs <- write.xlsx(split_data , glue('..\\data\\mid\\eve\\{Sys.Date()}拆解月份数据.xlsx'))
    ...
    split_data <- split_data %>% 
      ...
      mutate(note_real = if_else(round(real_amount_history , 2) > round(accrued_amount_history , 2) , '实收>应收' , '') ,
